...
import { useDisconnectShiprocket } from '@/hooks/useShiprocket';
...
export function ShippingSettings({ store }: ShippingSettingsProps) {
  ...
  const handleShiprocketToggle = async (checked: boolean) => {
    if (checked && !store.shiprocket_connected) {
      // Open modal to connect
      setShiprocketModalOpen(true);
    } else if (!checked && store.shiprocket_connected) {
      // Disconnect
      try {
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-4">
        <div className="space-y-0.5">
          ...
          {store.shiprocket_connected && store.shiprocket_email && (
            <p className="text-xs text-green-400 mt-1">
              Connected as {store.shiprocket_email}
            </p>
          )}
        </div>
        <Switch
          checked={store.shiprocket_connected || false}
